...
(deftest session-management
  ...
  (let [nyse-session (call (new-fix-session "nyse" "local" port-s "me" "you"))]
    (is (= (:id nyse-session) :me-you)))

  ; Create a session with the identical sender and target arguments.
  (is (thrown? Exception 
      (call (new-fix-session "nyse" "local" port-s "me" "you"))))

  ; Create another session.
  (let [nasdaq-session (call (new-fix-session "nasdaq" "foreign" port-s "him"
                                              "her"))]
  ...
  (let [cme-session (call (new-fix-session "cme" "galactic" port-s "up"
                                           "down"))]
